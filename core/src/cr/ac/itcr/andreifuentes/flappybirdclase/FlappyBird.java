...
public class FlappyBird extends ApplicationAdapter {
	...
	public FlappyBird(int opt) {
		/*
		*Recibe una sola variable para la dificultad lo cual permite
		* que dentro del codigo del render ocurran eventos si fuera el caso
		* no se me ocurrio algo mas que afectar la distancia entre los tubos y
		* separacion entre ellos ademas la velocidad con la que cae el flappy
		* */
		switch (opt){
			case 0:{
				this.dificultad = opt;
				this.gap = 800;
				this.reducedvelocity = 15;
                this.distance = Gdx.graphics.getWidth();
			}break;
			case 1:{
				this.dificultad = opt;
				this.gap = 500;
				this.reducedvelocity = 20;
				this.distance = Gdx.graphics.getWidth() * 4/5;
			}break;
			case 2:{
				this.dificultad = opt;
				this.gap = 300;
				this.reducedvelocity = 25;
				this.distance = Gdx.graphics.getWidth() * 3/5;
			}break;
		}
...
//						bottomTube.getHeight());
			...
			if (Gdx.input.justTouched()){
				velocity = velocity - reducedvelocity;
			}
